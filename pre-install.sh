...
aws route53 list-resource-record-sets --hosted-zone-id ${HOSTED_ZONE_ID} --output json | jq --arg Name $NAME '[.ResourceRecordSets[] | select(.Name | test($Name)) | {ResourceRecordSet: ., Action: "DELETE"}] | {Changes: ., Comment: "Delete record sets for $Name"}' > $JSON_FILE
